...
class AdService {
  ...
  Future<void> initialize() async {
    try {
      ...
      // Load ads setting from Firebase (with timeout)
      await _loadAdsEnabledFromFirebase().timeout(
        const Duration(seconds: 5),
        onTimeout: () {
          _adsEnabled = true; // Default to enabled on timeout
        },
      );
      ...
      // If anything fails, default to ads enabled
      _adsEnabled = true;
      loadInterstitialAd();
    }
  }
  ...
  Future<void> _loadAdsEnabledFromFirebase() async {
    try {
      ...
      if (snapshot.exists && snapshot.value != null) {
        _adsEnabled = snapshot.value as bool? ?? true;
      } else {
        // Default to true if not set, and create the setting
        _adsEnabled = true;
        await _database!.child('app_settings/ads_enabled').set(true);
      }
    } catch (e) {
      _adsEnabled = true; // Default to enabled on error
    }
  }
  ...
  void _listenToAdsEnabled() {
    ...
    _adsEnabledSubscription = _database!.child('app_settings/ads_enabled').onValue.listen((event) {
      if (event.snapshot.exists && event.snapshot.value != null) {
        _adsEnabled = event.snapshot.value as bool? ?? true;
