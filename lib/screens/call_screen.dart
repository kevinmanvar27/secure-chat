...
class CallScreen extends StatefulWidget {
  ...
  final CallType callType; // Type of call (chat/voice/video)
  ...
  const CallScreen({
    ...
    this.callType = CallType.video, // Default to video call
  });
...
class _CallScreenState extends State<CallScreen> with TickerProviderStateMixin {
  ...
  Future<void> _initializeOneToOneCallWithRemote(String remoteId) async {
    ...
    if (!widget.isRandomCall) {
      // For private calls, set mic/camera based on callType
      switch (widget.callType) {
        case CallType.chat:
          // Chat only - disable both mic and camera
          _webrtcService.setMicEnabled(false);
          _webrtcService.setCameraEnabled(false);
          _isMicOn = false;
          _isCameraOn = false;
          break;
        case CallType.voice:
          // Voice call - enable mic, disable camera
          ...
          break;
        case CallType.video:
          // Video call - enable camera, mic muted by default
          _webrtcService.setMicEnabled(false);
      ...
      _webrtcService.setCameraEnabled(true); // Camera on for random calls
      _isMicOn = true;
      _isCameraOn = true;
    }
