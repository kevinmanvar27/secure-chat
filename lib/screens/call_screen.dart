...
class _CallScreenState extends State<CallScreen> with TickerProviderStateMixin {
  ...
  Future<void> _initializeOneToOneCallWithRemote(String remoteId) async {
    ...
    if (!widget.isRandomCall) {
      _webrtcService.toggleMicrophone(); // Mute
      _isMicOn = false;
    } else {
      _isMicOn = true; // Random calls start unmuted
    }
    ...
    if (widget.isCaller) {
      _updateStatus('Calling $remoteId...');
      await _webrtcService.createOffer(effectiveRoomId, remoteUserId: remoteId);
    } else {
      _updateStatus('Waiting for $remoteId to call...');
          ...
          child: SafeArea(
            child: Stack(
              children: [
                ...
                // Local video preview - always visible (don't hide with UI)
                if (!_isChatVisible)
                  Positioned(
                    top: 60,
                    right: 20,
                ...
                if (!_isChatVisible && _isUIVisible)
                  Positioned(
                    top: 20,
                    left: 20,
                    right: 150, // Leave space for local video
                    child: Container(
                      padding: const EdgeInsets.symmetric(
                        horizontal: 16,
                        vertical: 12,
                    ...
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintStyle: TextStyle(
                            color: Colors.white.withOpacity(0.5), // hint color
                          ),
