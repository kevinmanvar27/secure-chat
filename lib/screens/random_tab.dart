...
class RandomTabState extends State<RandomTab> with WidgetsBindingObserver {
  ...
  Future<void> _startSearching() async {
    ...
    // Check if should show ad before searching
    if (_adService.shouldShowAdForRandomMatch()) {
      final adShown = await _adService.showInterstitialAd();
      if (adShown) {
        // Wait a moment after ad closes
  ...
  void _findMatch() {
    ...
    _searchTimer = Timer.periodic(const Duration(seconds: 1), (timer) async {
      if (!_isSearching || !_isTabActive || _isConnected) {
        timer.cancel();
        return;
      }
  ...
  Future<void> _checkForMatch() async {
    if (!_isSearching || !_isTabActive || _isConnected) return;
    ...
    try {
      ...
      if (matchResult && _isSearching && _isTabActive && !_isConnected) {
        _searchTimer?.cancel();
  ...
  Future<void> _initiateConnection(String remoteUserId, {required bool isCaller, String? roomId}) async {
    if (!_isTabActive || _isConnected) return;
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _isConnected 
                        ...
                        : (_isSearching ? Colors.orange.withOpacity(0.9) : Colors.grey.withOpacity(0.8)),
                  ...
                  child: Row(
                    ...
                    children: [
                      Icon(
                        _isConnected ? Icons.person : (_isSearching ? Icons.search : Icons.person_outline),
                        color: Colors.white,
                        size: 18,
                      ),
                      const SizedBox(width: 8),
                      Text(
                        _isConnected ? 'Connected' : (_isSearching ? 'Searching...' : 'Ready'),
